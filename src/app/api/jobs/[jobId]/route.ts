...
export async function PATCH(
    ...
    try {
        ...
        const { title, description, budget, status, deadline } = body
        ...
        if (job.campaign.client.agencyId !== session.agencyId && session.role !== 'SUPERADMIN' && !session.godMode) {
            return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
        }

        /* 5️⃣ VALIDATION */
        const parsedBudget = budget !== undefined ? Number(budget) : undefined
        ...
        const updatedJob = await prisma.job.update({
            ...
            data: {
                ...
                budget: parsedBudget,
                status: status !== undefined ? (status as JobStatus) : undefined,
