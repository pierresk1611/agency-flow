...
import { Trophy, Plus, FileText, ArrowRight } from 'lucide-react' // <--- PRIDANÝ Plus
...
export default async function TendersPage({ params }: { params: { slug: string } }) {
  ...
  // OCHRANA: Iba ADMIN/TRAFFIC/ACCOUNT
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b pb-4">
        <div>
            ...
            <p className="text-muted-foreground text-sm font-medium text-slate-500">
                Správa prebiehajúcich výberových konaní agentúry {agency.name}.
            </p>
        </div>
      ...
      <Card className="shadow-xl border-none ring-1 ring-slate-200 overflow-hidden">
        <CardHeader className="bg-slate-900 text-white py-4">
          <CardTitle className="text-xs font-black uppercase tracking-widest flex items-center gap-2">
            <Trophy className="h-4 w-4 text-yellow-400" /> Aktuálne príležitosti na trhu
          </CardTitle>
        </CardHeader>
            ...
            <Table>
                <TableHeader className="bg-slate-50">
                    <TableRow>
                        ...
                        <TableHead className="text-[10px] font-black uppercase">Termín doručenia</TableHead>
                        <TableHead className="text-[10px] font-black uppercase">Fee / Budget</TableHead>
                        <TableHead className="text-[10px] font-black uppercase">Dokumenty</TableHead>
                        <TableHead className="text-[10px] font-black uppercase text-right pr-6">Akcia</TableHead>
                    </TableRow>
                </TableHeader>
                <TableBody>
                {tenders.length === 0 ? (
                    <TableRow><TableCell colSpan={5} className="text-center py-20 text-slate-400 italic">Zatiaľ ste nezaevidovali žiadne tendre.</TableCell></TableRow>
                ) : (
                    tenders.map(t => (
                        <TableRow key={t.id} className="hover:bg-slate-50 transition-colors group">
                            <TableCell className="pl-6 py-4">
                                <div className="flex flex-col">
                                    ...
                                    {t.isConverted && <Badge className="w-fit mt-1 h-4 text-[8px] bg-green-100 text-green-700 border-none font-black uppercase">VYHRANÉ</Badge>}
                                </div>
                            </TableCell>
                            <TableCell className="text-sm font-medium">
                                <span className={new Date(t.deadline) < new Date() && !t.isConverted ? "text-red-600 font-bold" : "text-slate-600"}>
                                    {format(new Date(t.deadline), 'dd.MM.yyyy')}
                                </span>
                            </TableCell>
                            <TableCell className="font-mono text-sm font-bold text-slate-700">
                                {t.budget ? `${t.budget.toLocaleString()} €` : "0 €"}
                            </TableCell>
                            <TableCell>
                                <div className="flex items-center gap-1.5 text-xs text-slate-400 font-bold uppercase">
                                    <FileText className="h-3.5 w-3.5" /> {t._count.files} súborov
                                </div>
                            </TableCell>
