...
export default async function TendersPage({ params }: { params: { slug: string } }) {
  // 1. Session a autentifikácia
  const session = await getSession()
  if (!session) redirect('/login')

  // 2. Načítanie agentúry
  const agency = await prisma.agency.findUnique({ where: { slug: params.slug } })
  if (!agency) return notFound()

  // 3. Ochrana rolí: kreatívec vidí len svoje priradené tendre
  ...
  const tenders = await prisma.tender.findMany({
    where: { agencyId: agency.id, archivedAt: null, ...tenderFilter },
    include: {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b pb-4">
        ...
        {/* Tlačidlo pridania tendra len pre manažérov */}
        {!isCreative && (
                      ...
                      <TableCell className="text-right pr-6">
                        <Link href={`/${params.slug}/tenders/${t.id}`}>
                          <Button
                            variant="ghost"
                            size="sm"
                            className="text-purple-600 hover:text-purple-700 hover:bg-purple-50 font-bold transition-all"
                          >
