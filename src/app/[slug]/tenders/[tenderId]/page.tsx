...
export default async function TenderDetailPage({ params }: { params: { slug: string, tenderId: string } }) {
  ...
  // Načítanie tendra podľa tenderId
  ...
  // Kontrola, či tender existuje a patrí do správnej agentúry
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
                ...
                {tender.isConverted ? (
                    <Badge className="bg-green-600 text-white border-none px-3 font-bold">VYHRANÉ</Badge>
                ) : (
            ...
            <p className="text-[10px] text-slate-400 mt-1 font-mono uppercase tracking-widest font-bold">ID Inštancie: {tender.id}</p>
          </div>
        </div>

        {/* TLAČIDLO NA VÝHRU TENDRA */}
        ...
        <div className="lg:col-span-2 space-y-6">
            <Card className="shadow-xl border-none ring-1 ring-slate-200 overflow-hidden">
                ...
                <CardContent className="pt-8 pb-10 px-8">
                    <div className="bg-slate-50 p-8 rounded-2xl border border-dashed border-slate-300 text-slate-600 text-sm leading-relaxed italic">
                        Tu sa nachádza priestor pre hlavný brief tendra. Cieľom je vypracovať víťaznú ponuku s predpokladaným ročným fee vo výške <span className="font-bold text-slate-900">{tender.budget?.toLocaleString()} €</span>.
                    </div>
                </CardContent>
            </Card>
                ...
                <CardContent className="pt-6">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {tender.files.length === 0 ? (
                            <p className="text-xs text-slate-400 py-4 col-span-2 text-center border rounded-lg border-dashed">Zatiaľ neboli nahraté žiadne prílohy.</p>
                        ) : (
                            tender.files.map(f => (
            ...
            <Card className="shadow-xl border-none ring-1 ring-slate-200 overflow-hidden">
                ...
                <CardContent className="pt-6 space-y-4 px-6">
                    {tender.assignments.length === 0 ? (
                        <p className="text-xs text-slate-400 text-center py-4 italic">Nikto zatiaľ nie je priradený.</p>
                    ) : (
                ...
                <CardContent className="p-6 space-y-5">
                    <div className="flex justify-between items-center">
                        <div className="flex items-center gap-2 text-slate-400 text-xs font-bold uppercase tracking-tighter">
                            <Calendar className="h-4 w-4" /> Deadline odovzdania
                        </div>
                    ...
                    <div className="flex justify-between items-center">
                        <div className="flex items-center gap-2 text-slate-400 text-xs font-bold uppercase tracking-tighter">
                            <Euro className="h-4 w-4" /> Predpokladané Fee
                        </div>
