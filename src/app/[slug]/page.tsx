...
export default async function DashboardPage({ params }: { params: { slug: string } }) {
  ...
  try {
    ...
    // 3. BUDGET DÁTA PRE GRAF (Safe Mapping - konverzia na Number)
    ...
    if (!isCreative) {
      ...
      workloadData = allUsers.map(u => ({ name: u.name || u.email.split('@')[0], value: u._count?.assignments || 0 })).filter(v => v.value > 0)
    }
    ...
    return (
        <div className="p-20 text-center bg-white rounded-xl shadow-lg m-8">
            ...
            <p className="text-sm text-slate-500 mt-2">Príčina: Chyba dátového typu alebo prázdnej relácie.</p>
            <div className="mt-6 p-4 bg-slate-50 rounded text-left font-mono text-[10px] text-red-400 overflow-auto">
                {error.message || "Neznáma chyba servera"}
            </div>
            <Button onClick={() => window.location.reload()} className="mt-8 bg-slate-900 text-white">Obnoviť stránku</Button>
        </div>
    )
  }
}