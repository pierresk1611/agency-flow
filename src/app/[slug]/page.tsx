...
import { AlertCircle, Clock, TrendingUp, Users, Euro, CheckCircle2, ListChecks, Download, PieChart as PieIcon, BarChart3, Calendar } from "lucide-react"
...
export default async function DashboardPage({ params }: { params: { slug: string } }) {
  ...
  // 1. NAČÍTANIE JOBOV (Pre nadpisy, warningy, ale NEPREPOČÍTAVAME)
  const jobs = await prisma.job.findMany({
    where: { 
      archivedAt: null,
      campaign: { client: { agencyId: agency.isId } },
  ...
  // 2. Získanie dát pre PRVÝ RAD KARIET (Jednoduché count/sumy)
  ...
  // 3. Získanie dát pre ANALYTIKU (Zložitejšie)
  ...
  // 4. WORKLOAD A TIMESHEETY (Filtrovanie cez WHERE na najvyššej úrovni - toto je oprava)
  ...
  // ... (workloadData a creativeTimeData sú definované rovnako ako predtým, len sú podmienkou isCreative) ...
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="bg-amber-500 text-white shadow-lg border-none">
            <CardContent className="pt-4"><p className="text-[10px] font-bold uppercase opacity-80">Kritické</p><div className="text-2xl font-black">{warning.length}</div></CardContent>
        </Card>
