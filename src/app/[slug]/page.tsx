...
// Importy grafov (upevnenie, aby build prešiel)
...
export default async function DashboardPage({ params }: { params: { slug: string } }) {
  ...
  try {
    ...
    // 3. ANALYTIKA: BUDGETY (Pre Admin/Traffic)
    ...
    // 4. ANALYTIKA: VYŤAŽENOSŤ (Pre Admin/Traffic)
    ...
    if (!isCreative) {
      ...
      workloadData = users.map(u => ({ name: (u.name || u.email.split('@')[0]).substring(0, 12), value: u._count?.assignments || 0 })).filter(v => v.value > 0)
    }
          ...
          <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
            <Link href={`/${params.slug}/jobs`} className="block transform transition hover:scale-105">
                <Card className="bg-slate-900 text-white h-full shadow-lg border-none">
                    <CardContent className="pt-4"><p className="text-[10px] font-bold uppercase opacity-50 tracking-widest">Aktívne Joby</p><div className="text-2xl font-black">{jobs.filter(j => j.status !== 'DONE').length}</div></CardContent>
                </Card>
            </Link>
            <Card className="bg-red-600 text-white shadow-lg border-none">
                <CardContent className="pt-4"><p className="text-[10px] font-bold uppercase opacity-80 tracking-widest">Mešká</p><div className="text-2xl font-black">{overdue.length}</div></CardContent>
            </Card>
            <Card className="bg-amber-500 text-white shadow-lg border-none">
                <CardContent className="pt-4"><p className="text-[10px] font-bold uppercase opacity-80 tracking-widest">Kritické (7d)</p><div className="text-2xl font-black">{warning.length}</div></CardContent>
            </Card>
            ...
            <Card className={`lg:col-span-4 shadow-xl border-none ring-1 ring-slate-200 ${isCreative ? 'lg:col-span-12' : ''}`}>
                ...
                <CardContent className="pt-6">
                    ...
                    <div className="grid grid-cols-3 gap-2 w-full text-center mt-6">
                        <div className="bg-red-50 p-2 rounded-lg text-red-700 font-black text-xs">TODO: {statusCounts.TODO}</div>
            ...
            {!isCreative && (
                <>
                    <Card className="lg:col-span-6 shadow-xl border-none ring-1 ring-slate-200">
                        <CardHeader className="border-b bg-slate-50/50 py-3">
                            <CardTitle className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2"><Users className="h-3 w-3" /> Vyťaženosť tímu</CardTitle>
                        </CardHeader>
                        <CardContent className="pt-6"><WorkloadChart data={workloadData} slug={params.slug} /></CardContent>
                    </Card>

                    <Card className="lg:col-span-6 shadow-xl border-none ring-1 ring-slate-200">
                        <CardHeader className="border-b bg-slate-50/50 py-3"><CardTitle className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2"><CheckCircle2 className="h-3 w-3" /> Schvaľovanie výkazov</CardTitle></CardHeader>
    ...
    return (
        <div className="p-20 text-center bg-white rounded-xl shadow-lg m-8">
            ...
            <h2 className="text-xl font-black text-slate-900 uppercase">Chyba pri vykresľovaní Dashboardu!</h2>
            <p className="text-sm text-slate-500 mt-2">Prosím, skontroluj konzolu Vercel pre digest: {error.digest || 'N/A'}</p>
            <p className="text-xs text-slate-400 mt-2">Server chyba: {error.message}</p>
        </div>
    )
  }
}