...
import { LayoutDashboard, Briefcase, Clock, Users, LogOut, Settings, Trophy, Building2, CalendarDays, TrendingUp } from 'lucide-react'
...
export function Sidebar({ slug, role }: { slug: string; role: string }) {
  ...
  // ZABEZPEČENIE UNIKÁTNYCH CESTÍ SO SLUGOM
  const routes = [
  ...
  // Tieto sekcie vidia len Admini a vyššie
  if (role !== 'CREATIVE') {
      ...
      <div className="px-3 py-2 flex-1">
        <Link href={`/${slug}`} className="flex items-center pl-3 mb-10 hover:opacity-80 transition">
          <h1 className="text-xl font-bold italic">
            Agency<span className="text-blue-500 text-2xl">Flow</span>
          </h1>
        </Link>
          ...
          {routes.map((route) => (
            <Link
              ...
              className={cn(
                ...
                pathname === route.href || (pathname.startsWith(route.href) && route.href.length > 5 && route.href.split('/').length < pathname.split('/').length) ? 'text-white bg-white/20 shadow-sm' : 'text-zinc-400'
              )}
            >
